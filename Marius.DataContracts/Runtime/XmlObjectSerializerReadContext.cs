...
public class XmlObjectSerializerReadContext : XmlObjectSerializerContext
{
    ...
    private XmlDocument Document => _document ??= new XmlDocument();
    ...
    public void ResetCollectionMemberInfo()
    {
    ...
    [DoesNotReturn]
    public void ThrowSerializationException(string message) => throw new SerializationException(message);

    public static XmlObjectSerializerReadContext CreateContext(DataContractSerializer serializer, DataContract rootTypeDataContract)
    {
    ...
    public XmlObjectSerializerReadContext(XmlObjectSerializer serializer, int maxItemsInObjectGraph, StreamingContext streamingContext, bool ignoreExtensionDataObject)
        : base(serializer, maxItemsInObjectGraph, streamingContext, ignoreExtensionDataObject)
    {
    }

    public XmlObjectSerializerReadContext(DataContractSerializer serializer, DataContract rootTypeDataContract)
        : base(serializer, rootTypeDataContract)
    {
        attributes = new Attributes();
    }

    public T? Read<T>(XmlReaderDelegator xmlReader, string name, string ns, DataContract? contract)
    {
        contract ??= GetDataContract(typeof(T));
        return (T?)InternalDeserialize(xmlReader, name, ns, typeof(T), ref contract);
    }

    public virtual object? InternalDeserialize(XmlReaderDelegator xmlReader, Type declaredType, string? name, string? ns)
    {
        var dataContract = GetDataContract(declaredType);
        return InternalDeserialize(xmlReader, name, ns, declaredType, ref dataContract);
    }

    public virtual object? InternalDeserialize(XmlReaderDelegator xmlReader, Type declaredType, DataContract? dataContract, string? name, string? ns)
    {
    ...
    protected bool TryHandleNullOrRef(XmlReaderDelegator reader, string? name, string? ns, Type declaredType, ref object? retObj)
    {
        ...
        if (attributes.Ref != Globals.NewObjectId)
        {
            ...
            retObj = GetExistingObject(attributes.Ref, name, ns);
            reader.Skip();
            return true;
        }
    ...
    protected object? InternalDeserialize(XmlReaderDelegator reader, string? name, string? ns, Type declaredType, ref DataContract dataContract)
    {
        ...
        {
            var tempDataContract = ResolveDataContractFromKnownTypes(attributes.XsiTypeName, attributes.XsiTypeNamespace, dataContract, declaredType);

            dataContract = tempDataContract ?? throw XmlObjectSerializer.CreateSerializationException(XmlObjectSerializer.TryAddLineInfo(reader, SR.Format(SR.DcTypeNotResolvedOnDeserialize, attributes.XsiTypeNamespace, attributes.XsiTypeName, reader.NamespaceURI, reader.LocalName)));
        ...
        {
            var factoryDataContract = ResolveDataContractFromKnownTypes(attributes.FactoryTypeName, attributes.FactoryTypeNamespace, dataContract, declaredType);
        ...
        {
            var obj = ReadDataContractValue(dataContract, reader);
    ...
    public int GetMemberIndex(XmlReaderDelegator xmlReader, ImmutableArray<XmlDictionaryString> memberNames, ImmutableArray<XmlDictionaryString> memberNamespaces, int memberIndex, ExtensionDataObject? extensionData)
    {
    ...
    public int GetMemberIndexWithRequiredMembers(XmlReaderDelegator xmlReader, ImmutableArray<XmlDictionaryString> memberNames, ImmutableArray<XmlDictionaryString> memberNamespaces, int memberIndex, int requiredIndex, ExtensionDataObject? extensionData)
    {
        for (var i = memberIndex + 1; i < memberNames.Length; i++)
        {
            if (xmlReader.IsStartElement(memberNames[i], memberNamespaces[i]))
    ...
    [DoesNotReturn]
    public static void ThrowRequiredMemberMissingException(XmlReaderDelegator xmlReader, int memberIndex, int requiredIndex, ImmutableArray<XmlDictionaryString> memberNames)
    {
    ...
    public void HandleUnknownElement(XmlReaderDelegator xmlReader, ExtensionDataObject extensionData, int memberIndex)
    {
    ...
    public bool TryAddNewObject<T>(T value, [NotNullWhen(true)] out object? boxed)
        where T : struct
    {
        Debug.Assert(attributes != null);
        ...
        {
            boxed = value;
            DeserializedObjects.Add(attributes.Id, boxed);
            return true;
        }
    ...
    public void ReplaceDeserializedObject(string id, object? oldObj, object? newObj)
    {
        ...
        if (id != Globals.NewObjectId)
        {
            ...
            {
                var oldType = oldObj != null ? DataContract.GetClrTypeFullName(oldObj.GetType()) : SR.UnknownNullType;
                var newType = newObj != null ? DataContract.GetClrTypeFullName(newObj.GetType()) : SR.UnknownNullType;
    ...
    public T GetExistingObject<T>(string id, string? name, string? ns)
    {
        var retObj = DeserializedObjects.GetObject(id);
        if (retObj == null)
    ...
    public object GetExistingObject(string id, string? name, string? ns)
    {
        var retObj = DeserializedObjects.GetObject(id);
        if (retObj == null)
            throw XmlObjectSerializer.CreateSerializationException(SR.Format(SR.DeserializedObjectWithIdNotFound, id));
...
#pragma warning disable SYSLIB0050 // IObjectReference is obsolete
    ...
    {
        var realObj = obj.GetRealObject(GetStreamingContext());
...
#pragma warning restore SYSLIB0050
    ...
    public static void ParseQualifiedName(string qname, XmlReaderDelegator xmlReader, out string name, out string? ns, out string prefix)
    {
    ...
    public static T[] EnsureArraySize<T>(T[] array, int index)
    {
        if (array.Length <= index)
        {
            if (index == int.MaxValue)
                throw XmlObjectSerializer.CreateSerializationException(SR.Format(SR.MaxArrayLengthExceeded, int.MaxValue, DataContract.GetClrTypeFullName(typeof(T))));

            var newSize = index < int.MaxValue / 2 ? index * 2 : int.MaxValue;
    ...
    public object? ReadIXmlSerializable(XmlReaderDelegator xmlReader, XmlDataContract xmlDataContract, bool isMemberType)
    {
    ...
    public static object? ReadRootIXmlSerializable(XmlReaderDelegator xmlReader, XmlDataContract xmlDataContract, bool isMemberType)
    {
        return ReadIXmlSerializable(new XmlSerializableReader(), xmlReader, xmlDataContract, isMemberType);
    }

    public static object? ReadIXmlSerializable(XmlSerializableReader xmlSerializableReader, XmlReaderDelegator xmlReader, XmlDataContract xmlDataContract, bool isMemberType)
    {
        object? obj = null;
        ...
        {
            var xmlSerializable = xmlDataContract.Create();
...
#pragma warning restore SYSLIB0050
        ...
        while ((nodeType = xmlReader.MoveToContent()) != XmlNodeType.EndElement)
        {
            ...
            {
                xmlReader.Skip();
                value = GetExistingObject<object>(attributes.Ref, name, string.Empty);
            }
    ...
    public IDataNode? ReadExtensionDataValue(XmlReaderDelegator xmlReader)
    {
        ...
        {
            xmlReader.Skip();
            var o = GetExistingObjectOrExtensionData(attributes.Ref);
            dataNode = o as IDataNode ?? new DataNode<object>(o);
        ...
        else
        {
            ...
            {
                var dataContract = ResolveDataContractFromTypeName();
                ...
                else
                {
                    ...
                    else if (dataContract is EnumDataContract contract)
                    {
                        dataNode = new DataNode<object>(contract.ReadEnumValue(xmlReader));
                        InitializeExtensionDataNode(dataNode, dataContractName, dataContractNamespace);
                    }
    ...
    private IDataNode ReadExtensionDataValue(XmlReaderDelegator xmlReader, string? dataContractName, string? dataContractNamespace)
    {
        ...
        var namespacesInScope = xmlReader.GetNamespacesInScope(XmlNamespaceScope.ExcludeXml);
        Read(xmlReader);
        xmlReader.MoveToContent();
        ...
        switch (xmlReader.NodeType)
        {
            ...
            case XmlNodeType.EndElement:
            {
                ...
                var objNode = ReadUnknownPrimitiveData(xmlReader, Globals.TypeOfObject, dataContractName, dataContractNamespace);
                xmlReader.ReadEndElement();
    ...
    {
        var valueType = xmlReader.ValueType;
        ...
        var objNode = ReadUnknownPrimitiveData(xmlReader, valueType, dataContractName, dataContractNamespace);
        xmlReader.ReadEndElement();
        return objNode;
    }
    ...
    {
        var dataNode = xmlReader.ReadExtensionData(type);
        InitializeExtensionDataNode(dataNode, dataContractName, dataContractNamespace);
        return dataNode;
    }
    ...
    private CollectionDataNode ReadUnknownCollectionData(XmlReaderDelegator xmlReader, string? dataContractName, string? dataContractNamespace)
    {
        ...
        var arraySize = attributes.ArraySZSize;
        XmlNodeType nodeType;
        while ((nodeType = xmlReader.MoveToContent()) != XmlNodeType.EndElement)
        {
            if (nodeType != XmlNodeType.Element)
    ...
    private ISerializableDataNode ReadUnknownISerializableData(XmlReaderDelegator xmlReader, string? dataContractName, string? dataContractNamespace)
    {
        ...
        while ((nodeType = xmlReader.MoveToContent()) != XmlNodeType.EndElement)
        {
            ...
            {
                Value = ReadExtensionDataValue(xmlReader),
            };
    ...
    {
        var couldBeISerializableData = true;
        var couldBeCollectionData = true;
        var couldBeClassData = true;
        ...
        {
            xmlAttributes = new List<XmlAttribute>();
            foreach (var prefixNsPair in namespaces)
            {
        ...
        while ((nodeType = xmlReader.NodeType) != XmlNodeType.EndElement)
        {
            ...
            var childNode = Document.ReadNode(xmlReader.UnderlyingReader)!;
    ...
    public XmlReaderDelegator CreateReaderOverChildNodes(IList<XmlAttribute>? xmlAttributes, IList<XmlNode> xmlChildNodes)
    {
        var wrapperElement = CreateWrapperXmlElement(Document, xmlAttributes, xmlChildNodes, null, null, null);
        var nodeReader = CreateReaderDelegatorForReader(new XmlNodeReader(wrapperElement));
        nodeReader.MoveToContent();
        Read(nodeReader);
        return nodeReader;
    }

    public static XmlElement CreateWrapperXmlElement(XmlDocument document, IList<XmlAttribute>? xmlAttributes, IList<XmlNode>? xmlChildNodes, string? prefix, string? localName, string? ns)
    {
        localName ??= "wrapper";
        ns ??= string.Empty;
        var wrapperElement = document.CreateElement(prefix, localName, ns);
        if (xmlAttributes != null)
        {
            for (var i = 0; i < xmlAttributes.Count; i++)
            {
                wrapperElement.Attributes.Append(xmlAttributes[i]);
            }
        }
    ...
    {
        var attribute = string.IsNullOrEmpty(prefix) ? Document.CreateAttribute(null, Globals.XmlnsPrefix, Globals.XmlnsNamespace) : Document.CreateAttribute(Globals.XmlnsPrefix, prefix, Globals.XmlnsNamespace);
        attribute.Value = ns;
        return attribute;
    }

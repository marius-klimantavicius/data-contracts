...
internal sealed class ExtensionDataReader : XmlReader
{
    ...
    {
        var deserializedDataNode = _deserializedDataNodes == null || _deserializedDataNodes.Count == 0 ? null : _deserializedDataNodes.Dequeue();
    ...
    private bool IsXmlDataNode => _internalNodeType == ExtensionDataNodeType.Xml;

    public override XmlNodeType NodeType => IsXmlDataNode ? _xmlNodeReader.NodeType : _nodeType;
    public override string LocalName => IsXmlDataNode ? _xmlNodeReader.LocalName : _localName!;
    public override string NamespaceURI => IsXmlDataNode ? _xmlNodeReader.NamespaceURI : _ns!;
    public override string Prefix => IsXmlDataNode ? _xmlNodeReader.Prefix : _prefix!;
    public override string Value => IsXmlDataNode ? _xmlNodeReader.Value : _value!;
    public override int Depth => IsXmlDataNode ? _xmlNodeReader.Depth : _depth;
    public override int AttributeCount => IsXmlDataNode ? _xmlNodeReader.AttributeCount : _attributeCount;
    public override bool EOF => IsXmlDataNode ? _xmlNodeReader.EOF : _readState == ReadState.EndOfFile;
    public override ReadState ReadState => IsXmlDataNode ? _xmlNodeReader.ReadState : _readState;
    public override bool IsEmptyElement => IsXmlDataNode && _xmlNodeReader.IsEmptyElement;

    public override bool IsDefault => IsXmlDataNode ? _xmlNodeReader.IsDefault : base.IsDefault;

    //public override char QuoteChar { get { return IsXmlDataNode ? xmlNodeReader.QuoteChar : base.QuoteChar; } }
    public override XmlSpace XmlSpace => IsXmlDataNode ? _xmlNodeReader.XmlSpace : base.XmlSpace;
    public override string XmlLang => IsXmlDataNode ? _xmlNodeReader.XmlLang : base.XmlLang;
    public override string this[int i] => IsXmlDataNode ? _xmlNodeReader[i] : GetAttribute(i);
    public override string? this[string name] => IsXmlDataNode ? _xmlNodeReader[name] : GetAttribute(name);
    public override string? this[string name, string? namespaceURI] => IsXmlDataNode ? _xmlNodeReader[name, namespaceURI] : GetAttribute(name, namespaceURI);
    ...
    public override void MoveToAttribute(int index)
    {
        ...
        else
        {
            ...
            var attribute = _element.attributes![index];
    ...
    public override string? GetAttribute(string name, string? namespaceURI)
    {
        ...
        for (var i = 0; i < _element.attributeCount; i++)
        {
    ...
    private void MoveNext(IDataNode? dataNode)
    {
        switch (_internalNodeType)
        {
            ...
            case ExtensionDataNodeType.NullElement:
                ...
                return;
            default:
                var dataNodeType = dataNode?.DataType;
    ...
    private void MoveNextInClass(ClassDataNode dataNode)
    {
        ...
        if (dataNode.Members != null && _element.childElementIndex < dataNode.Members.Count)
        {
            ...
            var member = dataNode.Members[_element.childElementIndex++];
    ...
    private void MoveNextInCollection(CollectionDataNode dataNode)
    {
        ...
        if (dataNode.Items != null && _element.childElementIndex < dataNode.Items.Count)
        {
            ...
            var item = dataNode.Items[_element.childElementIndex++];
    ...
    private void MoveNextInXml(XmlDataNode dataNode)
    {
        ...
        else
        {
            ...
            var wrapperElement = XmlObjectSerializerReadContext.CreateWrapperXmlElement(dataNode.OwnerDocument, dataNode.XmlAttributes, dataNode.XmlChildNodes, _element?.prefix, _element?.localName, _element?.ns);
            if (_element != null)
            {
                for (var i = 0; i < _element.attributeCount; i++)
                {
                    var a = _element.attributes![i];
                    var xmlAttr = dataNode.OwnerDocument.CreateAttribute(a.prefix, a.localName!, a.ns);
    ...
    {
        var type = dataNode.DataType;
        var isTypedNode = true;
    ...
    private bool MoveToText(Type type, IDataNode dataNode, bool isTypedNode)
    {
        ...
        switch (Type.GetTypeCode(type))
        {
            ...
            case TypeCode.DateTime:
                var dateTime = isTypedNode ? ((DataNode<DateTime>)dataNode).GetValue() : (DateTime)dataNode.Value;
            ...
            default:
                ...
                {
                    var bytes = isTypedNode ? ((DataNode<byte[]>)dataNode).GetValue() : (byte[])dataNode.Value;
                ...
                {
                    var guid = isTypedNode ? ((DataNode<Guid>)dataNode).GetValue() : (Guid)dataNode.Value;
                    _value = guid.ToString();
                }
                else if (type == Globals.TypeOfUri)
                {
                    var uri = isTypedNode ? ((DataNode<Uri>)dataNode).GetValue() : (Uri)dataNode.Value;
    ...
    {
        ns ??= string.Empty;
        var prefix = (string?)s_nsToPrefixTable[ns];
        if (prefix == null)
        {
            lock (s_prefixLock)
            {

    ...
    private object? ReadItemOfPrimitiveType(XmlReaderDelegator xmlReader, XmlObjectSerializerReadContext context, Type type, string name, string ns, PrimitiveDataContract? primitiveContract, int nullables)
    {
        ...
        if (objectId != null)
        {
            if (objectId.Length == 0)
            {
                ...
                {
                    throw new SerializationException(SR.Format(SR.ValueTypeCannotHaveId, DataContractContext.GetClrTypeFullName(type)));
                }
            ...
            else
            {
                if (typeIsValueType)
                {
                    throw new SerializationException(SR.Format(SR.ValueTypeCannotHaveRef, DataContractContext.GetClrTypeFullName(type)));
                }
                else
                {
        ...
        else
        {
            ...
            {
                throw new SerializationException(SR.Format(SR.ValueTypeCannotBeNull, DataContractContext.GetClrTypeFullName(type)));
            }
            else
            {
                value = null;
    ...
    private static CollectionSetItemDelegate GetCollectionSetItemDelegate<T>(CollectionDataContract collectionContract, object resultCollectionObject, bool isReadOnlyCollection)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (addMethod == null)
                {
                    throw new InvalidDataContractException(SR.Format(SR.CollectionMustHaveAddMethod, DataContractContext.GetClrTypeFullName(collectionContract.UnderlyingType)));
                }

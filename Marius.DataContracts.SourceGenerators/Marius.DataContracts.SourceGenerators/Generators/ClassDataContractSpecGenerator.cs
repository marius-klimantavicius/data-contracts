...
internal class ClassDataContractSpecGenerator : SpecContractGenerator
{
    ...
    public override void DeclareDataContract()
    {
        ...
        AppendLine($"public static global::Marius.DataContracts.Runtime.ClassDataContract<{ClassContract.UnderlyingType.FullyQualifiedName}> {ClassContract.GeneratedName};");
    }
    ...
    public override (string localName, string? underlyingTypeOverride) GenerateDataContract(SourceProductionContext context, string xmlDictionary)
    {
        ...
        var generateMethod = LocalName($"Get{ClassContract.TypeInfoPropertyName}DataContract");
        using (NewWriter())
        {
            ...
            using (Block())
            {
                AppendLine($"private static global::Marius.DataContracts.Runtime.ClassDataContract<{classTypeName}> {generateMethod}(global::System.Xml.XmlDictionary {xmlDictionary}) => new global::Marius.DataContracts.Runtime.ClassDataContract<{classTypeName}>");
                using (Block(end: "};"))
                {
            ...
            context.AddSource($"DataContractContext.{Writer.FileName(ClassContract.TypeInfoPropertyName)}.g.cs", Writer.GetString());
        }

        AppendLine($"{ClassContract.GeneratedName} = {generateMethod}({xmlDictionary});");

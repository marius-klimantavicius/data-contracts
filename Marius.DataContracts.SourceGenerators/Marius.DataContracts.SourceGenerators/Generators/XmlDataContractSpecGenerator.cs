...
internal class XmlDataContractSpecGenerator : SpecContractGenerator
{
    ...
    {
        AppendLine($"public static global::Marius.DataContracts.Runtime.XmlDataContract<{XmlContract.UnderlyingType.FullyQualifiedName}> {XmlContract.GeneratedName};");
    }
    ...
    public override (string localName, string? underlyingTypeOverride) GenerateDataContract(SourceProductionContext context, string xmlDictionary)
    {
        ...
        var generateMethod = LocalName($"Get{XmlContract.TypeInfoPropertyName}DataContract");
        using (NewWriter())
        {
            ...
            using (Block())
            {
                AppendLine($"private static global::Marius.DataContracts.Runtime.XmlDataContract<{classTypeName}> {generateMethod}(global::System.Xml.XmlDictionary {xmlDictionary})");
                using (Block())
                {
                    ...
                    if (XmlContract.TopLevelElementName == null)
                        ...
                        AppendLine($"var {topLevelElementName} = {xmlDictionary}.Add({SymbolDisplay.FormatLiteral(XmlContract.TopLevelElementName, true)});");
                    ...
                    if (XmlContract.TopLevelElementNamespace == null)
                        ...
                        AppendLine($"var {topLevelElementNamespace} = {xmlDictionary}.Add({SymbolDisplay.FormatLiteral(XmlContract.TopLevelElementNamespace, true)});");
                    ...
                    using (Block())
                    {
                        ...
                        {
                            AppendLine($"{topLevelElementName} = {xmlDictionary}.Add({xmlName}.Name);");
                            AppendLine($"{topLevelElementNamespace} = ({xmlName}.Namespace == global::Marius.DataContracts.Runtime.Globals.SchemaNamespace) ? global::Marius.DataContracts.Runtime.DictionaryGlobals.EmptyString : {xmlDictionary}.Add({xmlName}.Namespace);");
                        }
                        else
                        {
                            if (string.IsNullOrEmpty(XmlContract.XmlRootAttribute.ElementName))
                                ...
                                AppendLine($"{topLevelElementName} = {xmlDictionary}.Add({SymbolDisplay.FormatLiteral(XmlContract.XmlRootAttribute.ElementName!, true)});");
                            ...
                            if (string.IsNullOrEmpty(XmlContract.XmlRootAttribute.Namespace))
                                ...
                                AppendLine($"{topLevelElementNamespace} = {xmlDictionary}.Add({SymbolDisplay.FormatLiteral(XmlContract.XmlRootAttribute.Namespace!, true)});");
                        }
                    }
                    ...
                    using (Block(end: "};"))
                    {
                        ...
                        AppendLine($"Name = {xmlDictionary}.Add({xmlName}.Name),");
                        AppendLine($"Namespace = {xmlDictionary}.Add({xmlName}.Namespace),");
                        ...
                        AppendLine("Read = static (xmlReader, context) => throw new global::System.NotImplementedException(),");
                    }
                }
            }

            context.AddSource($"DataContractContext.{Writer.FileName(XmlContract.TypeInfoPropertyName)}.g.cs", Writer.GetString());
        }

        AppendLine($"{XmlContract.GeneratedName} = {generateMethod}({xmlDictionary});");

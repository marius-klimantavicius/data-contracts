...
internal class EnumDataContractSpecGenerator : SpecContractGenerator
{
    ...
    {
        AppendLine($"public static global::Marius.DataContracts.Runtime.EnumDataContract<{EnumContract.UnderlyingType.FullyQualifiedName}> {EnumContract.GeneratedName};");
    }
    ...
    public override (string localName, string? underlyingTypeOverride) GenerateDataContract(SourceProductionContext context, string xmlDictionary)
    {
        ...
        var generateMethod = LocalName($"Get{EnumContract.TypeInfoPropertyName}DataContract");
        using (NewWriter())
        {
            ...
            using (Block())
            {
                AppendLine($"private static global::Marius.DataContracts.Runtime.EnumDataContract<{classTypeName}> {generateMethod}(global::System.Xml.XmlDictionary {xmlDictionary}) => new global::Marius.DataContracts.Runtime.EnumDataContract<{classTypeName}>");
                using (Block(end: "};"))
                {
            ...
            context.AddSource($"DataContractContext.{Writer.FileName(EnumContract.TypeInfoPropertyName)}.g.cs", Writer.GetString());
        }

        AppendLine($"{EnumContract.GeneratedName} = {generateMethod}({xmlDictionary});");
